...
if (conf.AUTOREACT_STATUS === "yes") {
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const dj = '923131613251';
            const dj2 = '923348585489';
            const dj3 = "923131613251";
            const luffy = '923348585489';
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*ADEEL XMD WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ ADEEL xmd is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ ADEEL xmd Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("ADEEL xmd is Online 🕸\n\n");
